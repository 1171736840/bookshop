...
function resetForm(){
	$("input[name='bookName'],input[name='bookAuthor'],input[name='bookPublish']").val("");
	return false;
}
function selectBook() {
	...
	function g() {
		...
		$.post(window.ctx + "/showAllBook", parameter, function(json) {
			...
			for(var i in bookList) {
				...
				bookListHTML += "<div class='min-page-photo'><a href='" + window.ctx + "/jsp/showBook?bookISBN=" + bookList[i].bookISBN + "' target='_blank'>";
				...
				bookListHTML += "<a class='bookName' href='" + window.ctx + "/jsp/showBook?bookISBN=" + bookList[i].bookISBN + "' target='_blank' title='" + bookList[i].bookName + "'>" + bookList[i].bookName + "</a>";
				bookListHTML += "<div><a class='bookPublish' href='" + window.ctx + "/jsp/showAllBook?bookPublish=" + bookList[i].bookPublish + "' target='_blank' title='" + bookList[i].bookPublish + "'>" + bookList[i].bookPublish + "</a>";
				bookListHTML += "<a class='bookAuthor' href='" + window.ctx + "/jsp/showAllBook?bookAuthor=" + bookList[i].bookAuthor + "' target='_blank' title='" + bookList[i].bookAuthor + "'>" + bookList[i].bookAuthor + "</a>";
			...
			$("#resultNumber2").html(json.resultNumber);
			$("#thisPageNumber2").html(json.thisPageNumber);
			$("#maxPageNumber2").html(json.maxPageNumber);
